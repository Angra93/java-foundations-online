...
//        watchesList.remove(0); <- удаляет по индексу
        ...
        System.out.println("watchesList.contains(gShock) = " + watchesList.contains(gShock)); // <- проверяет есть ли такой элемент
        System.out.println("watchesList.contains(seiko) = " + watchesList.contains(seiko)); // <- его удалили
        ...
        System.out.println("watchesList.size() = " + watchesList.size()); // <- состоит из seiko поэтому 1
